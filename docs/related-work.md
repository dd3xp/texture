...
#### 它根本没在做"在给定区域内填纹理"这件事（2026-08-21）

前面几轮一直在测纹理质量和背景污染，但更根本的问题是**输入形状本身没被保住**。
把输入 crate 轮廓降到各档分辨率取非白为前景，和输出前景算 IoU：

| 输出 | 输入前景占比 | 输出前景占比 | 轮廓 IoU | 轮廓被改掉 |
| --- | --- | --- | --- | --- |
| 16×16 | 56% | 100% | 0.56 | 44% |
| 32×32 | 56% | 72% | 0.78 | 22% |
| 64×64 | 56% | 65% | **0.86** | 14% |
| 128×128 | 56% | 100% | 0.56 | 44% |

若真在"给定区域内填纹理"，IoU 应为 1.00。最好一档只有 0.86。
输出里出现了带透视的开口箱、箱沿、投影——这些都是 SDXL 先验里
"木箱照片"该有的东西，不是输入那个圆角矩形。

#### 公平性验证：把 ControlNet 拉到 0.9（2026-08-21）

上面的判断只有在 baseline 拿到最强条件时才站得住。原配置
`controlnet_conditioning_scale` 只有 **0.15**（配置默认，canny-only）。
重跑一轮只改这一个值到 **0.9**，其余全部不变（`baselines/sdpixl/configs/probe_wood_cn09.yaml`）。
对照图 `experiments/sdpixl_sweep_cn09/montage_cn09.png`。

**结果：不但没救，反而更差。**

| 输出 | IoU @0.9 | IoU @0.15 | 变化 | 背景白色残留 @0.9 | @0.15 |
| --- | --- | --- | --- | --- | --- |
| 16×16 | 0.56 | 0.56 | ±0 | **0%** | 0% |
| 32×32 | 0.56 | 0.78 | **−0.22** | **0%** | 28% |
| 64×64 | 0.56 | 0.86 | **−0.30** | **0%** | 35% |
| 128×128 | 0.56 | 0.56 | ±0 | **0%** | 0% |

四档全部退化到 IoU 0.56、背景白色残留 0%——**整幅画布被涂满，没有任何一档例外**。
原本表现最好的 64×64 从 0.86 掉到 0.56。

看图能看出原因：条件强度拉高后，模型把输入轮廓那条 canny 边
**当成了画面内容里的一道边**，沿着它画出一圈木质边框，
于是每一档都变成"一个嵌在木框里的箱子"。它更听话了，但听的是错的话。

**结构性结论（这是 related work 里最有力的一条）：**
ControlNet canny 传达的是"**这里有一条边**"，不是"**这条边外面是背景、不要画**"。
SD-πXL 没有 mask、没有 alpha、没有任何区域语义通道，
所以"区域约束"这件事在它的架构里根本无法表达。
把条件强度调大只会让它把边画得更用力。**这不是调参问题。**

范围声明：本结论限于 SD-πXL 现有的 ControlNet 条件通道（canny/depth，任意强度）。
换成 inpainting 模型或显式 mask 条件是另一条路径，但那已经不是 SD-πXL 了。

#### 对项目的结论

**项目不需要重新设计，可以继续。** 经三轮实测（12 个 run）修正后，
可用的 motivation 按强度排序：

1. **它做的根本不是这个任务，而且调参救不了。** 轮廓 IoU 最好只有 0.86，
   两端 0.56。把 ControlNet 从 0.15 拉到 0.9 后**四档全部退化到 0.56、
   背景全灭**——朝"更该有用"的方向调，结果严格变差。
   根因是架构层面的：canny 传达"这里有边"，不传达"边外是背景"，
   而 SD-πXL 没有 mask / alpha / 任何区域语义通道。
   这条最有力，因为它不是"效果不够好"，而是"能力不存在"，
   且有反向调参实验堵住"你把 baseline 调废了"的质疑。
2. **平整区域在两端都保不住，且是两种不同的失败**（16 吞掉背景，128 噪声淹没背景）。
   有单变量证据和可量化指标（背景色残留率 0% / 28% / 35% / 0%）。
3. **成本与输出分辨率完全脱钩**（三轮共十二个 run，均约 200 分钟）。
   意味着给小 sprite 不会更便宜，与像素画工具的需求相反。
4. **motif 随分辨率漂移且不可控**（16 平面板条 → 64 正面箱 → 128 三维透视箱）。
   论据是"不可控"，不是"做不到"。
...
## 基线：降采样 + 量化 + 硬裁（2026-08-23）

S1 证明真人的低分辨率纹理与「降采样+量化」难以区分（见 `premise.md`），
于是把它实现成正式基线：高分辨率纹理 → 面积降采样 → 量化到给定调色板 →
硬裁进给定区域。代码 `baselines/downsample_quantize/`。

在与 SD-πXL 完全相同的任务上（crate 区域图 + wood9bg 调色板）：
...
IoU=1.0 是硬裁构造出来的，不是什么了不起的结果。它的意义在于**确认了
SD-πXL 那个失败的性质**：区域保真不是一个"难"的问题，而是一个
需要架构里有相应通道才能表达的问题。SD-πXL 没有那个通道，所以它拿 0.56；
一个三行的硬裁拿 1.00。

对比图 `experiments/dq_baseline/vs_sdpixl.png`。

**基线的硬限制**：需要现成的高分辨率纹理，不能按材质名生成。
SD-πXL 在这一点上更通用。因此完整的对手应当是
「SD 按 prompt 生成高分辨率纹理 → 降采样 → 量化 → 硬裁」，
本轮未验证（需要占 GPU）。


### 完整流水线：SDXL 生成 + 降采样 + 量化 + 硬裁（2026-08-23）

补上 S2 基线缺的那一环（不能按材质名生成），构成完整对手：

    prompt ──SDXL──> 1024² 纹理 ──面积降采样──> N×N ──量化到调色板──> ──硬裁进区域──>

代码 `baselines/downsample_quantize/full_pipeline.py`，
prompt 为 `"seamless wood plank texture, flat top down view, tileable material"`。
...
**端到端 11 秒**，含模型加载，四个分辨率一次全出。
对照 SD-πXL 的单分辨率 200 分钟，**每张输出快约 4400 倍**。

对比图 `experiments/dq_full/comparison.png`。

#### 这对项目意味着什么

本项目最初陈述的任务是：**输入纯色图 + 所需材质 → 输出该材质的像素画纹理，
任意分辨率**。这条流水线在四个分辨率上全部做到了轮廓 IoU 1.0、调色板完全合规，
耗时 11 秒。**任务本身已被解决。**

唯一还能看出的差距在**低分辨率的母题可读性**：16×16 和 32×32 那两格，
流水线输出的是"竖向条纹"，没有清晰的木板分界；
而以真人高分辨率纹理为源的 S2 基线（同样流水线，只换纹理源）
在同尺寸下有更明确的板缝结构。

但这个差距目前**只有视觉观察，没有测量**——S1 试过的七个特征
（颜色数/平坦度/边缘锐度/主周期/连通块密度/连通块大小/游程）
没有一个能把真人低分辨率纹理和降采样+量化分开。
**视觉上看得见、统计上测不出**，这个矛盾尚未解释，
它既可能说明特征选得不对，也可能说明视觉印象本身不可靠。
