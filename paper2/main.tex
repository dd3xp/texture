...
\item \textbf{Measurement.} To make those comparisons meaningful we repair
  ...
  roughly triples KID, and a pre-registered $2\times2$ confirms that the
  toroidal bias alone makes tiles seamless (\S\ref{sec:ablation}).
\end{itemize}
...
\paragraph{Pixel art and pixelization.} SD-$\pi$XL~\citep{binninger2024sdpixl}
optimises a palette-constrained small image per prompt by score
distillation~\citep{poole2023dreamfusion} from a latent diffusion
model~\citep{rombach2022ldm}, with no region channel or wrap-around.
Pixelization~\citep{gerstner2012pixelated,wu2022pixelization},
...
convert a given high-resolution image; none can change how many structural
units that image contains.

\paragraph{Tileable textures.} Exemplar-based~\citep{efros1999texture,efros2001quilting,gatys2015texture}
and procedural~\citep{perlin1985,worley1996} synthesis are not
text-conditioned. Diffusion methods enforce tileability at sampling time by
circular padding or rolled noise~\citep{zhou2022tilegen,vecchio2024controlmat,madar2025tiled},
and TexTile~\citep{rodriguezpardo2024textile} learns a tileability metric, all
at $\geq256$px; our baseline B7 uses the circular-padding recipe.

\paragraph{Conditioning and retrieval.} Colour prompts~\citep{butt2024colorpeel},
adapters~\citep{zhang2023controlnet} and retrieval-augmented
diffusion~\citep{blattmann2022rdm,sheynin2023knndiffusion,chen2023reimagen}
condition large generators; we retrieve only palettes and feed them as known
tokens, so structure is always generated.

\paragraph{Masked discrete generation.} MaskGIT~\citep{chang2022maskgit} and
its successors~\citep{chang2023muse,lezama2022tokencritic,li2023mage,weber2024maskbit}
decode VQ tokens~\citep{vandenoord2017vqvae} in parallel, a form of discrete
diffusion~\citep{austin2021d3pm}; pixel art is natively discrete and needs no
tokenizer. We restrict standard relative position
biases~\citep{shaw2018relative,raffel2020t5,liu2021swin,wu2021irpe} to the
torus, in the spirit of equivariant architectures~\citep{cohen2016gcnn}.

\paragraph{Judging generative models.} Pairwise VLM
judges~\citep{zheng2023mtbench} show positional bias~\citep{wang2024fair},
order-swap inconsistency~\citep{webdevjudge2025} and unreliable absolute
scores~\citep{kumar2026rankscore}, and
distribution metrics~\citep{heusel2017fid,binkowski2018kid,stein2023exposing,oquab2024dinov2}
...
colour a first-class input ($\Delta E=3.05$ vs $\approx15$), the toroidal bias
makes tiles seamless by construction, the palette memory supplies artist
ramps, and native generation is $28\times$ faster than the strongest
pipeline. These components apply wherever images are small, palette-indexed
or periodic, from sprites and tile maps to textile patterns.
