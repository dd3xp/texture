...
STUDIES = {
    ...
    # 判据继承 `analysis/paired/spread_recognise.py` 的预注册（`e9e3045`），
    # 此处不新设口径：只是把判官换成人，问的仍是**可辨认性**那条轴。
    "spread": {
        "csv": ROOT / "experiments/annotate/study_spread.csv",
        "target": "after", "other": "before",
        "title": "拉亮度跨度：更容易认出吗（各向同性那半，判官 10/11 待人确认）",
        "hit": ("人也认为重标版更容易认出 —— 各向同性那半**不再是全关**，"
                "这条路可以进交付（对门拒绝材质加一个可选的重标步骤）。"),
        "miss": ("人没确认。判官那 10/11 只能记作**判官单方结果**，"
                 "这条路带证据地关闭；正文凡涉及它的说法都要注明未经人工确认。"),
        "strata": [("16px", "16px"), ("24px", "24px"), ("32px", "32px")],
    },
}
