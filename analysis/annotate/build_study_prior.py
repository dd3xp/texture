...
from structural_prior import learn_prior, make_seed, fill_from_seed  # noqa: E402
...
def main() -> None:
    ...
    for m in cands:
        ...
        priors[m] = pr
        (seeded if pr["rows"] else plain).append(m)
    print(f"可比材质 {len(cands)}：有周期 {len(seeded)}，无周期 {len(plain)}")
