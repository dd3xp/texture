...
**第一次尝试（scale=0.8）操作检验未通过**（中位 27.0→8.1 看着很大，但逐材质
只有 6/10 变小，符号检验 p=0.754：brick 30→4.5 完全跟随，log cabin 26→79 反向）。
按判据两条结论都不能下，数据存为 `controlnet_units_s08.json`。
**停止规则（写在第二次运行之前）**：再试一次 `--scale 1.0`，其余一切不变。
若符号检验仍 p≥0.05，则判为「条件控制在两种强度下都不能稳定改变单元数」，
**这条路关闭，不再调参数试第三次**——否则就是在挑一个好看的结果。
"""
import argparse
import json
import sys
