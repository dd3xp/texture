...
**操作检验（两条都要过，缺一即作废）**
  (a) 出厂瓦片的接缝比 ratio 下降：逐材质配对符号检验 p<0.05 且方向为降。
      ⚠ **事后发现 (a) 的零假设写错了**（判据不改，但读的时候要知道）：
      `ratio_seam` 是约 3930 个可行位置里的 **argmin**，`ratio_center` 是一个固定点，
      符号检验 p=0.5 的零假设对这种比法不成立。正确的对照是**随机可行位置**，
      见 `seam_null.py`（预注册 `863ba3f`）：换了零假设后 22/22、35/35 与 p 值
      **逐位不变**，结论站得住；但效应量要报 2.76->0.85 / 2.99->1.04。
