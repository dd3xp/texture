...
def main():
    ...
    print("真人瓦片里，一个结构周期实际占几个像素（自相关主周期，单位=输出像素）")
    ...
    for n in sorted(by):
        ...
        stats[n] = float(np.median(pers))
        print(f"{n:>6}{len(by[n]):>8}{len(pers):>12}{np.median(pers):>10.1f}"
              f"{f'[{q1:.1f}, {q3:.1f}]':>16}{np.median(pers)/n:>12.2f}")
