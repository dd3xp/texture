...
**本脚本的假设（先注册，后跑）**：若模型没有尺度概念，它会在 32 的画布上画**16px 那种粒度**
的结构 —— 即结构主周期（像素）不随画布放大，于是每张瓦片里的结构单元数翻倍，
正好回到本项目诊断大模型失败时的那个病（真人约 3.2 单元/瓦片，大模型约 25）。
（跑完后判为**不成立**：过门瓦片的周期比 16->32 = 2.00，正比。）

⚠ **本段原写着"TRD 的训练数据全是 16x16，24/32 是零样本"，那是错的**（2026-09-12 更正）：
`runs/trd_v8|v10|v11d/config.json` 全是 `sizes:[16,32]`，`p32` 分别 0.3 / 0.5 / 0.7 ——
**32px 一直在训练，占三到七成的批次**；零样本的只有 24px。判读规则与结论都不依赖这句话
（它只是动机），但别再拿"32px 是零样本"当任何推理的前提，见
`analysis/arch/train_pool_structure.py`。

⚠⚠ **本脚本报的"真人 32px"（门 64% / 各向异性 0.534）是一个包，不是真人 32px**
（2026-09-14 更正，`analysis/arch/ref_pack_audit.py` 零 GPU 零 API 可复算）：
那个参照组 = `val32` 与 `V_mat` 的交集，**只有 74 张，其中 66 张（89%）来自
`ROllerozxa__mtg_tiled_32x` 一个包**（包名里就写着 tiled）。整份 val32 是门 46% / **0.132**，
与本脚本给 TRD 量到的 28% / **0.130** 是一回事。参照组比语料库里任何一组都更有结构——
**包括真人自己的 16px 验证集（0.197，MW p=0.055）**。
→ 下面"32px 上 TRD 塌掉、真人不塌"的那个差距，**量的是风格差异，不是能力差距**；
本脚本的逐张数字都对，**结论那一句不对**。判官那条（32px 输 41%、p=0.014）是另一把尺子，不受影响。

⚠⚠⚠ **本脚本的跨方法比较已作废（2026-09-14，`analysis/arch/scale_free_diag.py`，预注册 `35cd091`）**：
下面的周期比只在**过门的瓦片**上算，而这把筛子在两组上的通过率差三倍——
`TRD32_rr4/32` 过门率 **27.6%**、`B2val/32` **74.4%** → 选择偏差压过了要测的量。
换一把**无门、无阈值、覆盖 100% 瓦片**的尺子（`edge` = 环面相邻格颜色不同的比例）重测，
得到的是**相反**的读数：本脚本说 TRD 比 B2 更"按画布放大"（周期比 2.00 > B2 的 1.75），
无门的尺子说 **TRD 几乎不放大**（`edge32/edge16` 中位 0.954 vs B2 0.840，配对 p=4.2e-10），
且 TRD 在 32px 上比 B2 **显著更花**（配对符号检验 p=7e-05 验证集 / 1.7e-08 测试集），16px 上无差异。
**逐张周期数字仍然是对的；跨方法的那几句结论以新脚本为准。**
