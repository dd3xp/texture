...
----
## 后记（2026-09-15 追加；以上预注册原文一个字未动）

1. ⛔ 上面"为什么问这个"里那句 **"训练包里 16px 瓦片比 32px 多约 5 倍 → 16px 的先验赢"
   已撤回**（(M10)，`5042ca5`）：5 倍是**分析口径**，训练实际用 `extra=` 两个包
   （16px 3177 / 32px 1877 = 1.69 倍），而且瓦片数根本不决定梯度份额
   （`train_trd.py:491` 的 `--p32 0.7` + `--batch 256/--batch32 64` → 按格子 32px 是 16px 的 9.3 倍）。
   **主判决不依赖这句话**（它只是动机里的一句因果猜想），(M9) 的判决本身不受影响。
2. ⛔ 本文件产出的 **L 与 R 的数值/CI 一律不许引**（(M12)，`5042ca5`）：L16 是悬崖统计量，
   16px 的 Â 被 4 格周期主导，半高阈离 Â(2) 只有 0.005。**只许引方向。**
   方向另有一把**不含阈值**的尺子支持（`analysis/arch/gen_scale.py` 的对比 C，
   Δ=+0.0123 CI [+0.0039,+0.0183]）——但那用的是同一批瓦片，独立的只是汇总统计量。
"""
import argparse
import json
import sys
