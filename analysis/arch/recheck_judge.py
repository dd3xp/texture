...
EXPECT = {
    ...
    # 验证集 16px 上界臂：「挑哪张调色板」这条轴判官看不看得见（预注册 16ad1a4，(M52)）。免门（`--no_gate`），
    # A = palette=real（oracle，参照瓦片自己的调色板）、B = palette=xmodal（正式配置，前 5 随机），
    # 两边同模型/同配置/**同一张网格**，唯一变量是调色板来源 ⇒ 这条轴上任何可达做法的上界。
    # 可解率 95/125 = **76.0%**（全项目最高，成对刺激好判）vs 地板 17.8%（p=1.2e-45）。
    # 判决 AXIS_TOO_SMALL（59/95 = 62.1% < 行动线 0.65，虽 p=0.023 显著高于 0.5）。
    # ⚠ 引本臂必须同引：Jeffreys CI [0.521,0.714] **含 0.65** ⇒ ⛔ 不许读成"已排除上界 ≥65%"。
    "judge_full_pal_real_vs_pal_xmodal_16_V_mat.json":        (59,  95, 30, 0),
}
...
PILOT_EXPECT = {
    ...
    # b2_canvas 第一次（预注册 92fc3c7）：不过门，判据一条都没轮到。这 15 对**已整个作废**
    # ——第二次开臂（557a50e）免门重问了全部 272 对，一个旧答案都没复用。留在这里只为可复算。
