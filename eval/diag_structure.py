...
def main():
    ...
    ap.add_argument("--size", type=int, default=16, help="24 没有训练数据，是零样本")
    a = ap.parse_args()
    dev = "cuda"
    ...
    for rep in range(a.n):
        ...
        pal, grid = sample(model, temb, ks, n=a.size, cfg=a.cfg, choice_temp=a.choice_temp, steps=a.steps,
    ...
    tag = f"{a.run.name}_{a.ckpt[:-3]}_ct{a.choice_temp}_s{a.steps}_p{a.pal_top_p}_n{a.size}"
    out = ROOT / f"experiments/diag_struct_{tag}.png"
    up = max(1, 96 // a.size)
    Image.fromarray(sheet).resize((sheet.shape[1] * up, sheet.shape[0] * up), Image.NEAREST).save(out)
    res = {"run": str(a.run), "ckpt": a.ckpt, "step": ck["step"], "val_grid": tg / len(val),
           ...
           "choice_temp": a.choice_temp, "steps": a.steps, "cfg": a.cfg, "pal_top_p": a.pal_top_p, "size": a.size}
