...
2. **先量可解率，再看胜负**（`pilot` 子命令）：取前 N 对真题 + M 对**空对照**（两边放同一张图），
   **只记录每对是否两序一致，不记录谁赢**（字段白名单强制）。
   ...
   20 条臂的 100 对空对照合并为 **12%**（`analysis/arch/pilot_gate_audit.py`，p vs 50% = 1.9e-15），
   反推 q ~ 0.94 —— 两图相同时判官几乎总挑同一个**位置**，换序就翻面。
...
def main():
    ...
    if a.cmd == "pilot":
        ...
        if not ok:
            # 20 条试点合并的空对照实测 12/100 = 12%（analysis/arch/pilot_gate_audit.py），
            # 不是 2q(1-q) 那个 50% 的上界 -> 可解率 60% 出头**远高于地板**，只是没到门槛。
            print("  注意：不过门槛 != 判官没分辨力。这台仪器的可解率地板实测约 12%（合并 100 对空对照），"
