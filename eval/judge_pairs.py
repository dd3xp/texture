...
2. **先量可解率，再看胜负**（`pilot` 子命令）：取前 N 对真题 + M 对**空对照**（两边放同一张图），
   **只记录每对是否两序一致，不记录谁赢**（字段白名单强制）。
   空对照的一致率就是这台仪器的地板。⚠ 理论上界是 2q(1-q) ≤ 0.5，但**实测远低于它**：
   去重后 90 道空对照题合并为 **16%**（`analysis/arch/pilot_gate_audit.py`，p vs 50% = 1.8e-11），
   反推 q ~ 0.91 —— 两图相同时判官几乎总挑同一个**位置**，换序就翻面。
   所以"可解率 60% 出头"是**远高于地板**、只是没到门槛，**不等于判官没分辨力**。
   **真题可解率须 ≥ `--min_rate`（默认 0.65）且明显高于空对照**，才允许跑 `full`。
   ⚠ **空对照跨臂会重复**：下面第 122 行的取题只看 A 臂和可比对张数，所以同一张消融表里
   各条臂拿到的是**同一批空对照**（16px 消融表 25 条记录其实只有 5 道题）。合并多条臂的空对照
   **必须按 (A 臂, 对号) 去重**，否则会把重复当独立（旧口径"100 对 = 12%"就是这么来的）。
   建新表时若要让空对照跟着 B 臂走，改这里的取题并**在预注册里写明**。
...
def main():
    ...
    if a.cmd == "pilot":
        ...
        print(f"可解率：真题 {rr:.0%}（n={len(real)}）  空对照 {nr:.0%}（n={len(null)}）  "
              f"门槛 {a.min_rate:.0%} -> "
              f"{'**通过**，可以跑 full' if ok else '**不通过**：没到我们要的功效，不跑 full'}")
        if not ok:
            # 去重后 90 道空对照题实测 14/90 = 16%（analysis/arch/pilot_gate_audit.py），
            # 不是 2q(1-q) 那个 50% 的上界 -> 可解率 60% 出头**远高于地板**，只是没到门槛。
            print("  注意：不过门槛 != 判官没分辨力。这台仪器的可解率地板实测约 16%（去重后 90 道空对照），"
